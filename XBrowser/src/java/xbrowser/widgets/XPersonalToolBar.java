...
public class XPersonalToolBar extends JToolBar implements XBookmarkFolderListener
{
	...
	XAbstractBookmark abs_bm;
		...
		for( int i=0; i<components.length; i++ )
		{
			if( components[i] instanceof XPersonalBookmarkFolderButton )
				abs_bm = ((XPersonalBookmarkFolderButton)components[i]).getBookmarkFolder();
			else
				abs_bm = ((XPersonalBookmarkButton)components[i]).getBookmark();
	...
	int i;
		...
		if( new_abs_bm instanceof XBookmark)
			btn = new XPersonalBookmarkButton( (XBookmark)new_abs_bm );
		else if( new_abs_bm instanceof XBookmarkFolder )
			btn = new XPersonalBookmarkFolderButton( (XBookmarkFolder)new_abs_bm );
		else
			return;

	Dimension size = btn.getPreferredSize();

		XComponentBuilder.getInstance().buildFlatLook(btn, size.width, 25);
		add(btn, i);
	}
	...
	int i;
		...
		if( new_abs_bm instanceof XBookmark )
			folder_menu.insert( new XBookmarkMenuItem((XBookmark)new_abs_bm), i);
		else if( new_abs_bm instanceof XBookmarkFolder )
			folder_menu.insert( new XBookmarkFolderMenu((XBookmarkFolder)new_abs_bm), i);
	}

	private XBookmarkFolderMenu getBookmarkMenu(XBookmarkFolder bookmark_folder, XBookmarkFolderMenu base_menu)
	{
		if( base_menu.getBookmarkFolder()==bookmark_folder )
			return base_menu;
	...
	XBookmarkFolderMenu menu;
		...
		for( int i=0; i<components.length; i++ )
		{
			if( components[i] instanceof XBookmarkFolderMenu )
			{
				...
				if( menu.getBookmarkFolder()==bookmark_folder )
					return menu;
				else
				{
	...
	{
		if( parent.isPersonalFolder() )
			addToToolBar(abs_bm);
		else if( parent.isUnderPersonalFolder() )
		{
		Component[] components = getComponents();
		JMenu menu;
	...
	public void personalFolderChanged(XBookmarkFolder old_folder, XBookmarkFolder new_folder)
	{
		...
		Iterator it = old_folder.getBookmarks();
			...
			while( it.hasNext() )
				removeFromToolBar( (XAbstractBookmark)it.next() );
		}
		...
		Iterator it = new_folder.getBookmarks();
			...
			while( it.hasNext() )
				bookmarkAdded( (XAbstractBookmark)it.next(), new_folder );
		}
	...
	private class XPersonalBookmarkButton extends JButton implements ActionListener, PropertyChangeListener
	{
		public XPersonalBookmarkButton(XBookmark bookmark)
		{
			...
			setIcon(XComponentBuilder.getInstance().buildImageIcon(XPersonalToolBar.this, "image.Bookmark"));
	...
	private class XPersonalBookmarkFolderButton extends JButton implements ActionListener, PropertyChangeListener
	{
		public XPersonalBookmarkFolderButton(XBookmarkFolder bm_folder)
		{
			...
			setIcon(XComponentBuilder.getInstance().buildImageIcon(XPersonalToolBar.this, "image.BookmarkFolder"));
