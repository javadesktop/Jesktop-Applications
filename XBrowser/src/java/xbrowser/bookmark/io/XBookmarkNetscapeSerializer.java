...
public class XBookmarkNetscapeSerializer extends XBookmarkSerializer
{
	...
	private class XCallback extends HTMLEditorKit.ParserCallback
	{
		...
		public void handleText(char[] data, int pos)
		{
			if( currentBookmark!=null )
				currentBookmark.setTitle( new String(data) );
		}
	}
	...
	Iterator bookmarks = root_folder.getBookmarks();
		...
		while( bookmarks.hasNext() )
			saveBookmarksImpl( (XAbstractBookmark)bookmarks.next(), file_writer, "\t");
	...
	private void saveBookmarksImpl(XAbstractBookmark abs_bm, FileWriter file_writer, String header)
	{
		try
		{
			if( abs_bm instanceof XBookmark )
				saveBookmark( (XBookmark)abs_bm, file_writer, header);
			else if( abs_bm instanceof XBookmarkFolder )
				saveBookmarkFolder( (XBookmarkFolder)abs_bm, file_writer, header);
		}
		catch( Exception e )
		{
		}
	...
	private void saveBookmarkFolder(XBookmarkFolder bm_folder, FileWriter file_writer, String header) throws Exception
	{
		...
		if( bm_folder.getBookmarkCount()==0 )
			file_writer.write(header+"<DD>\n\n");
		else
		{
		...
		Iterator bookmarks = bm_folder.getBookmarks();
			...
			while( bookmarks.hasNext() )
				saveBookmarksImpl( (XAbstractBookmark)bookmarks.next(), file_writer, header+"\t");
